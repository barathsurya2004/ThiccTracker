...
const ExerciseMode: React.FC = () => {
  ...
  const { plans, activePlanId, quickWorkoutDay, quickWorkoutPlanName, currentExerciseIndex, currentSet } = useWorkoutStore();
        ...
        <section className="space-y-4">
          {currentDay.exercises.map((ex, i) => {
            ...
            return (
              <article
                key={i}
                className={`p-5 rounded-3xl transition-all duration-200 active:scale-[0.98] shadow-sm border flex gap-5 ${isCompleted
                  ? 'bg-surface-container-high border-surface-container text-on-surface-variant'
                  : 'bg-white border-surface-container-low hover:border-primary/20'
                  }`}
              >
                <div className={`w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0 shadow-inner flex items-center justify-center ${isCompleted ? 'bg-surface-container-highest text-on-surface-variant/40' : 'bg-surface-container text-primary/20'
                  }`}>
                  <Dumbbell size={40} />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className={`font-headline text-lg font-bold mb-3 uppercase tracking-tight break-words ${isCompleted ? 'text-on-surface-variant' : 'text-on-surface'}`}>
                    {ex.name}
                  </h3>
                  <div className="grid grid-cols-3 gap-2">
                    <div className={`rounded-xl p-2 text-center border ${isCompleted ? 'bg-surface-container/70 border-surface-container-high' : 'bg-surface-container-low/50 border-white'}`}>
                      <span className="block text-[8px] uppercase tracking-widest text-on-surface-variant font-black opacity-50">Sets</span>
                      <span className={`text-sm font-black ${isCompleted ? 'text-on-surface-variant' : 'text-primary'}`}>{ex.sets}</span>
                    </div>
                    <div className={`rounded-xl p-2 text-center border ${isCompleted ? 'bg-surface-container/70 border-surface-container-high' : 'bg-surface-container-low/50 border-white'}`}>
                      <span className="block text-[8px] uppercase tracking-widest text-on-surface-variant font-black opacity-50">Reps</span>
                      <span className={`text-sm font-black ${isCompleted ? 'text-on-surface-variant' : 'text-primary'}`}>{ex.reps}</span>
                    </div>
                    <div className={`rounded-xl p-2 text-center border ${isCompleted ? 'bg-surface-container/70 border-surface-container-high' : 'bg-surface-container-low/50 border-white'}`}>
                      <span className="block text-[8px] uppercase tracking-widest text-on-surface-variant font-black opacity-50">Rest</span>
                      <span className={`text-sm font-black ${isCompleted ? 'text-on-surface-variant' : 'text-primary'}`}>{ex.setRest}s</span>
                    </div>
                  </div>
                </div>
              </article>
